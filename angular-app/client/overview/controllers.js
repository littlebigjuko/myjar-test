...
angular.module('client.overview.controllers', []).
    ...
    ['$scope', '$rootScope', 'loanFactory', 'ModalService', '$http', function ($scope, $rootScope, loanFactory, ModalService, $http) {
        ...
        $scope.getColor = function (value) {
            ...
            return productColors[value];
        };

        $http.get('/myjar/api/api.php').success(function (response) {
            $scope.products = response;
            $scope.selectProduct(response[0]);
        });

        $scope.isSelectedProduct = function (value) {
            return $scope.selectedProduct.value == value ? "active" : "";
        }
        $scope.selectProduct = function (product) {
            switch (product.value) {
                case 2:
                case 3:
                    $scope.daySliderVisible = false;
                    $scope.earlierPaymentDate = moment().add(product.value, 'months').format('YYYY-MM-DD');
                    break;
                default:
                case 1:
                    $scope.daySliderVisible = true;
